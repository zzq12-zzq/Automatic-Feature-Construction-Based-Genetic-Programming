...
def eaSimple(population, toolbox, cxpb, mutpb, elitpb, ngen, randomseed, stats=None,
    ...
    sub_hof={}
    for i in population:
        i.fitness.values,b = toolbox.evaluate(individual=i, hof=[],sub_hof=sub_hof)
        sub_hof.update(b)
    ...
    for gen in range(1, ngen + 1):
        ...
        # print(offspringE[0],offspringE[0].fitness.values)
        # Evaluate the individuals with an invalid fitness
        invalid_ind = [ind for ind in offspring if not ind.fitness.valid]
        # print(len(invalid_ind))
        for i in invalid_ind:
            i.fitness.values,b = toolbox.evaluate(individual=i, hof=cop_po,sub_hof=sub_hof)
            sub_hof.update(b)
